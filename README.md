...
| R4 | `rules/R4-git.md` | git 管理：修改前置检查、跟踪范围、忽略策略 |
...
| `dev-docs/<项目名>/…` | 项目文档库 | 建真实项目时按目标结构创建 | **已落地**（2026-08-19：`/opt/doc` 重构为 `KH810` 结构；`common/` 待用户检查确认后删除） |
...
> 已于 2026-08-19 同步：R5、R6、本 README 中的 `dev-docs/features/<功能名>/` 路径已更新为 `dev-docs/<项目名>/features/<功能名>/`；`/opt/doc` 已按此结构落地（`KH810`）。
